...
function triangle(value1, type1, value2, type2) {
    ...
    if (!validTypes.includes(type1)) {
        console.error(`❌ Невідомий тип: '${type1}'.\n   Допустимі типи: ${validTypes.join(", ")}`);
        return "failed";
    }
    if (!validTypes.includes(type2)) {
        console.error(`❌ Невідомий тип: '${type2}'.\n   Допустимі типи: ${validTypes.join(", ")}`);
        return "failed";
    }
    let a = 0; // катет a
    let b = 0; // катет b
    let c = 0; // гіпотенуза
    let alpha = 0; // кут α (протилежний до катета a), градуси
    let beta = 0; // кут β (протилежний до катета b), градуси
    if (type1 === "leg" && type2 === "leg") {
        ...
        const leg = type1 === "leg" ? value1 : value2;
        const ang = type1 === "adjacent angle" ? value1 : value2;
        if (leg <= 0)
            return `❌ Помилка: катет має бути додатним числом. Отримано ${leg}`;
        if (ang <= 0 || ang >= 90)
        ...
        const leg = type1 === "leg" ? value1 : value2;
        const ang = type1 === "opposite angle" ? value1 : value2;
        if (leg <= 0)
            return `❌ Помилка: катет має бути додатним числом. Отримано ${leg}`;
        if (ang <= 0 || ang >= 90)
            return `❌ Помилка: кут має бути в межах (0°; 90°). Отримано ${ang}°`;
        ...
        const hyp = type1 === "hypotenuse" ? value1 : value2;
        const ang = type1 === "angle" ? value1 : value2;
        if (hyp <= 0)
            return `❌ Помилка: гіпотенуза має бути додатним числом. Отримано ${hyp}`;
        if (ang <= 0 || ang >= 90)
            return `❌ Помилка: кут має бути в межах (0°; 90°). Отримано ${ang}°`;
        ...
        console.log("   Допустимі комбінації: (leg, leg), (leg, hypotenuse),\n" +
